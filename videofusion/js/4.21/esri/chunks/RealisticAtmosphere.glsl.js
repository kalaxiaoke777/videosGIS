...
//>>built
define(["exports","../views/3d/webgl-engine/core/shaderLibrary/util/ColorConversion.glsl","../views/3d/webgl-engine/core/shaderModules/interfaces","../views/3d/webgl-engine/core/shaderModules/ShaderBuilder"],function(d,f,b,g){function e(c){const a=new g.ShaderBuilder;a.attributes.add("position","vec2");a.attributes.add("uv0","vec2");a.varyings.add("worldRay","vec3");a.varyings.add("vtc","vec2");c.haze&&a.varyings.add("eyeDir","vec3");a.vertex.uniforms.add("projectionInverse","mat4");a.vertex.uniforms.add("viewInverse",
...
a.fragment.code.add(b.glsl`const float krESun = 0.075;
...
#define oneOverScaleDepth vec2(atmosphereParameters1.w, atmosphereParameters2.w)`);c.haze||a.fragment.code.add(b.glsl`#define g atmosphereParameters2.x
#define gSq atmosphereParameters3.x
#define miePhaseCoefficients atmosphereParameters3.y
#define lowerAlphaBlendBound atmosphereParameters3.z`);a.fragment.code.add(b.glsl`
  ...
  `);c.haze||a.fragment.code.add(b.glsl`float surfaceBlend = 0.0;
...
""}
  ...
  `);c.haze&&a.fragment.code.add(b.glsl`vec3 end = cameraPosition + worldSpaceRay * rayEnd;
...
}`);a.fragment.code.add(b.glsl`vec2 opticalStartDepth = exp(scatterDistance * oneOverScaleDepth);
...
vec3 samplePoint = start + sampleRay * 0.5;`);c.haze?a.fragment.code.add(b.glsl`float cameraAngle = dot(-worldSpaceRay, end) / length(end);
...
vec2 scaledValuesDepth = scaledValues * shellDepth;`):a.fragment.code.add(b.glsl`float cameraAngle = dot(worldSpaceRay, start / length(start));
float angleMultiplier = cameraAngle > 0.0 ? cameraAngle : 0.0;
float scaleCameraAngle = scale(cameraAngle);
vec2 cameraOffset = scaleCameraAngle * opticalStartDepth * shellDepth;`);a.fragment.code.add(b.glsl`vec3 frontColor = vec3(0.0);
...
vec2 depth = exp(-altitude * scaleOverScaleDepth);`);c.haze?a.fragment.code.add(b.glsl`vec2 scatter = depth * scaledValuesDepth - cameraOffset;`):a.fragment.code.add(b.glsl`float lightAngle = dot(lightingMainDirection, samplePoint) / height;
float cameraAngle = dot(worldSpaceRay, samplePoint) / height;
float tmpScaledValues = scale(lightAngle) - scale(cameraAngle);
vec2 scatter = cameraOffset + tmpScaledValues * depth * shellDepth;`);a.fragment.code.add(b.glsl`attenuate = exp(-scatter.x * invWavelengthScaled);
...
float LdotRSq = LdotR * LdotR + 1.0;`);c.haze?a.fragment.code.add(b.glsl`vec3 colorCoefficients = (scaledLength * 0.75 * LdotRSq) * (krESun * invWavelength + kmESun );
vec3 color = colorCoefficients * frontColor;
vec3 colorBlue = colorCoefficients * frontColorBlue;`):a.fragment.code.add(b.glsl`vec3 rayleighCoefficients = (scaledLength * 0.75 * LdotRSq * krESun) * invWavelength;
...
vec3 colorBlue = rayleighCoefficients * frontColorBlue + mieCoefficients * frontColorBlue;`);a.fragment.code.add(b.glsl`vec3 ldrBlue = expTM(colorBlue, 2.0 * exposure);
vec3 ldrRed = expTM(color, exposure);
vec3 LDR = mix(ldrBlue, ldrRed, 0.2);`);c.haze?a.fragment.code.add(b.glsl`LDR *= (1.0 - cameraAngle);
vec3 hsv = rgb2hsv(LDR);
hsv.y = clamp(hsv.y * 1.5, 0.0, 1.0);
LDR = hsv2rgb(hsv);
vec3 finalColor = LDR;`):a.fragment.code.add(b.glsl`vec3 ldrReinhard = reinhardTM(color, exposure);
...
hsv.y = clamp(hsv.y * 1.5, 0.0, 1.0);
finalColor = hsv2rgb(hsv);`);a.fragment.code.add(b.glsl`
