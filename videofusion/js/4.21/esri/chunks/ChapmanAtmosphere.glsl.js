...
//>>built
define("exports ../views/3d/webgl-engine/core/shaderLibrary/attributes/TextureCoordinateAttribute.glsl ../views/3d/webgl-engine/core/shaderLibrary/output/ReadLinearDepth.glsl ../views/3d/webgl-engine/core/shaderLibrary/shading/Gamma.glsl ../views/3d/webgl-engine/core/shaderModules/interfaces ../views/3d/webgl-engine/core/shaderModules/ShaderBuilder".split(" "),function(d,f,g,h,a,k){function e(b){const c=new k.ShaderBuilder;c.attributes.add("position","vec2");c.include(f.TextureCoordinateAttribute,
{attributeTextureCoordinates:1});c.varyings.add("worldRay","vec3");c.varyings.add("eyeDir","vec3");c.vertex.uniforms.add("projectionInverse","mat4");c.vertex.uniforms.add("viewInverse","mat4");c.vertex.code.add(a.glsl`void main(void) {
vec3 posViewNear = (projectionInverse * vec4(position, -1, 1)).xyz;
eyeDir = posViewNear;
worldRay = (viewInverse * vec4(posViewNear, 0)).xyz;
forwardTextureCoordinates();
gl_Position = vec4(position, 1, 1);
}`);c.fragment.uniforms.add("lightingMainDirection","vec3").add("radii","vec2").add("scaleHeight","float").add("cameraPosition","vec3").add("nearFar","vec2").add("heightParameters","vec4").add("innerFadeDistance","float").add("altitudeFade","float").add("strength","float").add("depthTex","sampler2D").add("betaRayleigh","vec3").add("betaCombined","vec3").add("skyStrength","float").add("skyHazeMultiplier","float").add("hazeStrength","float").add("mieStrength","float").add("skyHazePow","float").add("sunSize",
"float").add("sunSpread","float");b.simplified||c.fragment.uniforms.add("betaMie","float");c.include(h.Gamma);b.haze&&c.fragment.include(g.ReadLinearDepth);c.fragment.code.add(a.glsl`vec2 sphereIntersect(vec3 start, vec3 dir, float radius, bool planet) {
float a = dot(dir, dir);
...
}`);c.fragment.code.add(a.glsl`float chapmanApproximation(float X, float h, float cosZenith) {
float c = sqrt(X + h);
float cExpH = c * exp(-h);
if (cosZenith >= 0.0) {
...
}`);c.fragment.code.add(a.glsl`float getOpticalDepth(vec3 position, vec3 dir) {
...
return scaleHeight * chapmanApproximation(radii[0] / scaleHeight, h / scaleHeight, dot(normalize(position), dir));
    ...
    ${b.simplified?"":a.glsl`const int STEPS = 6;`}
    ...
    vec3 getSkyColour(vec3 cameraPos, vec3 rayDir, vec3 lightDir, float terrainDepth) {
      ...
      ${b.simplified?"":a.glsl`float start = insideAtmosphere ? 0.0 : rayAtmosphereIntersect.x;`}
      ...
      if (heightParameters[0] < reducedPlanetRadius) {
        ...
        ${b.simplified?"":a.glsl`start = rayPlanetIntersect.y;`}
      }
      ...
      ${b.simplified?a.glsl`
          ...
          ${b.haze?a.glsl`if (terrainDepth != -1.0) { opticalDepth = min(0.25 * end, opticalDepth); }`:""}

          vec3 scattering =
          ${b.haze?a.glsl`betaRayleigh`:a.glsl`mix(0.5 * betaRayleigh, betaCombined,(pow(1.0 - clamp(dot(normalize(samplePoint), rayDir), 0.0, 1.0), 10.0)))`}
          * opticalDepth;
          vec3 lightDepth = betaCombined * lightOpticalDepth;
          vec3 absorption = (exp(-scattering) - exp(-lightDepth)) / (lightDepth - scattering);`:a.glsl`
      ...
      ${b.simplified?a.glsl``:a.glsl`
          float phaseRayleigh = 0.05968310365 * (1.0 + mumu);`}
        ${b.haze?a.glsl`
            ${b.simplified?a.glsl`
                return absorption * scattering;`:a.glsl`
                return scattering * stepSize * phaseRayleigh * betaRayleigh;`}`:a.glsl`
            const float g = 0.8;
            const float gg = g * g;
            ...
            ${b.simplified?a.glsl`
                return absorption * scattering * (1.0 + mieStrength * 0.15 * phaseMie);`:a.glsl`
    ...
    void main() {
      ...
      ${b.haze?a.glsl`
          ...
          ${b.simplified?a.glsl`
              vec3 col = ((depthSample != vec4(0)) ? 0.075 : skyHaze * 0.01) * hazeStrength * skyCol;`:a.glsl`
              vec3 col = ((depthSample != vec4(0)) ? 1.0 : skyHaze) * hazeStrength * skyCol;`}
          float alpha = 1.0;`:a.glsl`
          vec3 up = normalize(cameraPosition);
          ${b.simplified?a.glsl`
              vec3 col = mix(1.0, 3.0, max(0.0, dot(rayDir, up))) * skyStrength * skyCol;`:a.glsl`
  ...
  `);return c}var l=Object.freeze({__proto__:null,build:e});d.ChapmanAtmosphereShader=l;d.build=e});